...
static int	ft_check_front_dots(int pos, char *str, int deep, int max)
{
	int i;
	int	dots;
...
int			ft_check_front_up(char *str, int i, int size);
{
	int n;
    int max;
	...
	n += ft_check_back_dots(i, str, 3, max);
	if (n >= 1 && i - size >= 0 && str[i - size] == '.')
	{
